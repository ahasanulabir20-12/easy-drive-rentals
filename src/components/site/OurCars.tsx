...
type Category = {
  ...
  tone: "liquid-dark" | "liquid-green" | "liquid-gold";
};
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-5">
          {CATS.map((c, i) => (
            <article
              ...
              className={`liquid-glass ${c.tone} rounded-3xl p-6 animate-fade-in ${c.tone === "liquid-gold" ? "text-brand-black" : "text-white"}`}
            >
              <div className="flex items-center justify-between">
                <span className={`liquid-glass ${c.tone === "liquid-gold" ? "liquid-dark" : "liquid-gold"} rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-wider`}>
                  {t(`cat_${c.key}` as any)}
                </span>
                <CarIcon type={c.iconType} className={`h-10 w-14 ${c.tone === "liquid-gold" ? "text-brand-black" : "text-brand-yellow"}`} />
              </div>

              <h3 className="mt-4 font-display font-bold text-3xl">{t(`cat_${c.key}` as any)}</h3>
              <p className={`mt-1 text-sm ${c.tone === "liquid-gold" ? "text-brand-black/70" : "text-white/70"}`}>{t(`cat_${c.key}_d` as any)}</p>

              <ul className="mt-5 space-y-2.5 text-sm">
                <Row icon={<Users className="h-4 w-4" />} label={t("seats")} value={c.seats} dark={c.tone === "liquid-gold"} />
                <Row icon={<Snowflake className="h-4 w-4" />} label="AC" value={c.ac === "dual" ? t("ac_dual") : t("ac_single")} dark={c.tone === "liquid-gold"} />
                <Row icon={<Cog className="h-4 w-4" />} label={t("engine")} value={c.engine} dark={c.tone === "liquid-gold"} />
                <Row icon={<Fuel className="h-4 w-4" />} label={t("fuel")} value={c.fuel} dark={c.tone === "liquid-gold"} />
              </ul>
            </article>
          ))}
        </div>
...
const Row = ({ icon, label, value, dark = false }: { icon: React.ReactNode; label: string; value: string; dark?: boolean }) => (
  <li className={`flex items-center justify-between gap-3 border-b pb-2 last:border-none ${dark ? "border-brand-black/10" : "border-white/10"}`}>
    <span className={`inline-flex items-center gap-2 ${dark ? "text-brand-black/70" : "text-white/70"}`}>{icon} {label}</span>
    <span className={`font-semibold ${dark ? "text-brand-black" : "text-brand-yellow"}`}>{value}</span>
  </li>
);
